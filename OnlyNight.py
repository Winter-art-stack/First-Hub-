...
class AIWorker(QObject):
    ...
    def _build_prompts(self):
        if self.mode == "check":
            ...
            s = "Bạn là trợ lý AI thân thiện tích hợp trong sản phẩm đầu tiên của Night Kanz Nary. Trả lời tiếng Việt, ngắn gọn, rõ ràng ,hữu ích."
            u = self.prompt
        else:
    ...
    def _claude(self, sys_p, usr_p):
        ...
        if not key:
            return "[Chưa nhập Claude API Key trong Cài đặt]"
        ...
        if r.status_code != 200:
            return f"[Lỗi Claude {r.status_code}]: {r.text[:300]}"
        data = r.json()
    ...
    def _deepseek(self, sys_p, usr_p):
        ...
        if not key:
            return "[Chưa nhập DeepSeek API Key trong Cài đặt]"
        r = requests.post("https://api.deepseek.com/chat/completions",
            ...
            timeout=60)
        if r.status_code != 200:
            return f"[Lỗi DeepSeek {r.status_code}]: {r.text[:300]}"
        return r.json()["choices"][0]["message"]["content"]

    def _chatgpt(self, sys_p, usr_p):
        key = self.cfg.get("openai_key","").strip()
        if not key:
            return "[Chưa nhập OpenAI API Key trong Cài đặt]"
        r = requests.post("https://api.openai.com/v1/chat/completions",
            ...
            timeout=60)
        if r.status_code != 200:
            return f"[Lỗi OpenAI {r.status_code}]: {r.text[:300]}"
        return r.json()["choices"][0]["message"]["content"]
...
class MainWindow(QWidget):
    ...
    def _pg_settings(self):
        ...
        note = QLabel(
            ...
            "Thanks For Use My New Tool .... See Ya 😘"
        )
